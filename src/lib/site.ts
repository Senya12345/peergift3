/** Single source of truth for the things that appear in metadata, schema and copy. */
export const SITE = {
  name: 'Gamble Atlas',
  /** Overridden per environment; the placeholder makes an unset SITE_URL obvious. */
  url: process.env.SITE_URL ?? 'https://gambleatlas.example',
  tagline: 'Crypto gambling ratings aggregator',
  description:
    'Gamble Atlas combines the published ratings of several independent casino review platforms into one score per crypto casino, and links back to every figure so it can be checked.',
  email: 'pencasino@gmail.com',
  locale: 'en',
} as const;

/** Platforms whose published scores feed the aggregate. Named on /methodology/. */
export const SOURCE_PLATFORMS = [
  'AskGamblers',
  'Casino Guru',
  'Trustpilot',
  'Casinomeister',
  'LCB',
  'Bitcoin.com Games',
] as const;

export function absolute(path: string): string {
  return new URL(path, SITE.url).href;
}
