...
Only touch the Caddyfile again if the block's *content* actually changed — which means a
new casino, a changed `affiliateUrl`, or a header tweak. Most redeploys are just new static
files under `/var/www/gambleatlas/dist`, which Caddy serves immediately with no reload at
all. To see whether anything moved:

```bash
diff <(sed -n '/^# --- gambleatlas\.com/,/^# --- end gambleatlas\.com ---/p' /etc/caddy/Caddyfile) \
     deploy/gambleatlas-site.caddy
```

Note the pattern is anchored and stops at `gambleatlas\.com` rather than matching the whole
`# --- gambleatlas.com ---` string: the generated opening line carries a "generated by"
notice after the domain, so an exact-string match silently finds nothing and the diff comes
back looking like the entire block is new.

If it did change, swap the block rather than editing it by hand — the file also holds
another project's sites, so a slip here takes them down too:

```bash
cp /etc/caddy/Caddyfile /etc/caddy/Caddyfile.bak.$(date +%F-%H%M)
sed -i '/^# --- gambleatlas\.com/,/^# --- end gambleatlas\.com ---/d' /etc/caddy/Caddyfile
cat deploy/gambleatlas-site.caddy >> /etc/caddy/Caddyfile
caddy validate --config /etc/caddy/Caddyfile
systemctl reload caddy
```

`caddy validate` runs before the reload on purpose: a syntax error caught there changes
nothing, while the same error after a reload takes the other project's sites down with
ours. If validate fails, the timestamped backup next to the file is the way back.
